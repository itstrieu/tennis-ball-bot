...
class MotionController:
    ...
    def __init__(self, config=None):
        ...
        try:
            ...
            raise RobotError(
                f"GPIO initialization failed: {str(e)}", "motion_controller"
            )
        ...
        for motor_pins in self.config.pins.values():
            if isinstance(motor_pins, dict):
                ...
                for pin_name, pin in motor_pins.items():
                    ...
                    if result < 0:
                        self.logger.error(
                            f"Failed to claim pin {pin_name} ({pin}): {result}"
                        )
                    else:
                        self.logger.debug(
                            f"Successfully claimed pin {pin_name} ({pin})"
                        )
            else:
    ...
    async def initialize(self):  # Async initialize remains
        ...
        try:
            ...
            raise RobotError(
                f"Motion controller initialization failed: {str(e)}",
                "motion_controller",
            )
